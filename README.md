...
In that case, you will need to use a fake local domain (`dev.tusky.org`) and make your computer think that the domain is is served by the custom IP (e.g. `192.168.99.150`).

If you used the default CORS enabled domains, `dev.tusky.org` was configured to be allowed. If you want a custom one, you need to add it to the list in the variable `BACKEND_CORS_ORIGINS` in the `.env` file.

* Open your `hosts` file with administrative privileges using a text editor:
  * **Note for Windows**: If you are in Windows, open the main Windows menu, search for "notepad", right click on it, and select the option "open as Administrator" or similar. Then click the "File" menu, "Open file", go to the directory `c:\Windows\System32\Drivers\etc\`, select the option to show "All files" instead of only "Text (.txt) files", and open the `hosts` file.
  * **Note for Mac and Linux**: Your `hosts` file is probably located at `/etc/hosts`, you can edit it in a terminal running `sudo nano /etc/hosts`.

* Additional to the contents it might have, add a new line with the custom IP (e.g. `192.168.99.150`) a space character, and your fake local domain: `dev.tusky.org`.

The new line might look like:

```
192.168.99.100    dev.tusky.org
```

* Save the file.
  * **Note for Windows**: Make sure you save the file as "All files", without an extension of `.txt`. By default, Windows tries to add the extension. Make sure the file is saved as is, without extension.

...that will make your computer think that the fake local domain is served by that custom IP, and when you open that URL in your browser, it will talk directly to your locally running server when it is asked to go to `dev.tusky.org` and think that it is a remote server while it is actually running in your computer.

To configure it in your stack, follow the section **Change the development "domain"** below, using the domain `dev.tusky.org`.

After performing those steps you should be able to open: http://dev.tusky.org and it will be server by your stack in `localhost`.
...
```bash
STACK_NAME=stag-tusky-org sh ./scripts/deploy.sh
```
...
services:
  db:
    ...
    deploy:
      placement:
        constraints:
          - node.labels.tusky-org.app-db-data == true
```
...
```bash
docker node update --label-add tusky-org.app-db-data=true dog.example.com
```
...
Frontend: https://tusky.org

Backend: https://tusky.org/api/

Automatic Interactive Docs (Swagger UI): https://tusky.org/docs

Automatic Alternative Docs (ReDoc): https://tusky.org/redoc

PGAdmin: https://pgadmin.tusky.org
...
Frontend: https://stag.tusky.org

Backend: https://stag.tusky.org/api/

Automatic Interactive Docs (Swagger UI): https://stag.tusky.org/docs

Automatic Alternative Docs (ReDoc): https://stag.tusky.org/redoc

PGAdmin: https://pgadmin.stag.tusky.org
...
Frontend: http://dev.tusky.org

Backend: http://dev.tusky.org/api/

Automatic Interactive Docs (Swagger UI): https://dev.tusky.org/docs

Automatic Alternative Docs (ReDoc): https://dev.tusky.org/redoc

PGAdmin: http://dev.tusky.org:5050

Flower: http://dev.tusky.org:5555
